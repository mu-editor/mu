"""pgzhelper - Enhance Pygame Zero with additional capabilities.
    ...
    def obb_rect(x, y, w, h, angle, rx, ry, rw, rh):
        ...
        for obb_line in obb_lines:
            ...
            for rect_line in rect_lines:
                ...
                uA = ((l2x2 - l2x1) * (l1y1 - l2y1) -
                      (l2y2 - l2y1) * (l1x1 - l2x1)) / determinant
                if uA < 0 or uA > 1:
                    continue
        ...
        for rect in rects:
            ...
            for obb_line in obb_lines:
                ...
                for rect_line in rect_lines:
                    ...
                    uB = (l1x2_l1x1 * (l1y1 - l2y1) -
                          l1y2_l1y1 * (l1x1 - l2x1)) / determinant
                    if uB < 0 or uB > 1:
                        continue
    ...
    def load_images(self, sheet_name: str, cols: int, rows: int, cnt: int = 0, subrect: pygame.Rect = None):
        ...
        for col in range(0, cols):
            for row in range(0, rows):
                ...
                self._subrects[col + row * cols] = (int(col * width),
                                                    int(row * height), int(width), int(height))
    ...
    def next_image(self) -> int:
        if self._subrects is not None:
            ...
            if (self.image in self._images):
                next_image_idx = (self._images.index(
                    self.image) + 1) % len(self._images)
                self._image_idx = next_image_idx
        ...
        if subr != self._subrect:
            ...
            if self._subrect is not None:
                ...
                if surf_name not in self._orig_surfs:
                    self._orig_surfs[surf_name] = loaders.images.load(
                        self.image).subsurface(subr)
