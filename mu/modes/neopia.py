...
class NeopiaMode(BaseMode):
    ...
    # The below list defines the supported devices
    valid_boards = [
    ...
    def run_script(self):
        ...
        if tab.path:
            ...
            self.runner = self.view.add_python3_runner(
                ...
                script_name=tab.path,
                working_directory=cwd,
                interactive=False, # After finishing code, don't return to REPL state
                envars=envars,
            )
    ...
    # Added below member functions for changing mode by USB dongle detection
    # Originally the member functions are in MicroPythonMode
