"""
A mode for working with Mini SAM M4 using Adafuit's line of Circuit Python.
...
class MiniSAMMode(MicroPythonMode):
    """
    Represents the functionality required by the Mini SAM mode.
    """

    name = _('Mini SAM CircuitPython')
    description = _("Use CircuitPython on Mini SAM line of boards.")
    icon = 'minisam'
    save_timeout = 0  #: Don't autosave on Mini SAM boards. Casues a restart.
    connected = True  #: is the board connected.
    force_interrupt = False  #: NO keyboard interrupt on serial connection.
    valid_boards = [
        (0x1209, 0x2017),  # Mini SAM M4 CircuitPython
    ]
        ...
        buttons = [
            {
                ...
                'description': _('Open a serial connection to the Mini SAM.'),
    ...
    def workspace_dir(self):
        ...
        if device_dir:
            ...
            if self.connected:
                m = _('Could not find an attached Mini SAM '
                      ' device.')
