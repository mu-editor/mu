...
def test_run_blocking():
    ...
    output = p.run_blocking(sys.executable, ["-c", "import sys; print(sys.executable)"]).strip()
...
def test_run_blocking_timeout():
    ...
    #
    expected_output = ""
    output = p.run_blocking(sys.executable, ["-c", "import time; print(time.sleep(0.2))"], wait_for_s=0.1).strip()
    assert output == expected_output

#
# We have to devise a means of testing within a Qt event loop
#
@pytest.mark.skip("We have to devise a means of testing within a Qt event loop")
def test_run():
